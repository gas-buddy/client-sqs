...
describe('SQS Client', () => {
  ...
  test('Verify dead-letter queue handling', async () => {
    const qConfig: SQSClientConfiguration<'testQueue'> = {
      endpoints: {
        default: {
          accountId: process.env.SQS_ACCOUNT_ID || '000000000000',
    ...
    const dlqError = new Error('Dead-letter queue error');
    // @ts-ignore
    dlqError.deadLetter = true;

    global.fetch = jest.fn(() => Promise.reject(dlqError)) as any;
